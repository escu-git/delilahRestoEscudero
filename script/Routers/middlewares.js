...
const middlewares = {
    receivedData: async(req, res, next)=>{
        let errors = [];
        const {user, name, surname, email, phone, address, password} = req.body;
        if(!user, !name, !surname, !email, !phone, !address, !password) errors.push('You must complete all the user fields');
        if(password.length<8) errors.push('Password must be 8 characters');
        if(isNaN(phone)) errors.push('Phone must contain only numbers');
        if(Number(name || surname)) errors.push('Name must not contain numbers');
